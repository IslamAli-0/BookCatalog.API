...
* **Why `IExceptionHandler` over custom middleware:** The interface is the idiomatic .NET 8+ approach. It integrates with the existing `ProblemDetails` infrastructure, participates cleanly in DI, and is more testable than a raw middleware class. The older middleware approach required manual JSON serialization and pipeline wiring.
